...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name)) {
        ...
        // Cek duplikat
        $check = $pdo->prepare("SELECT COUNT(*) FROM categories WHERE category_name = ?");
        $check->execute([$name]);
        ...
        if ($check->fetchColumn() > 0) {
            ...
            // Insert kategori
            $stmt = $pdo->prepare("
                INSERT INTO categories (category_name, category_icon, category_description, is_active) 
                VALUES (?, ?, ?, ?)
            ");
            $stmt->execute([$name, $icon_name, $description, $is_active]);
...
<head>
    ...
    <title>Tambah Kategori - SISKATRA</title>
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <textarea name="category_description" 
                              placeholder="Deskripsi singkat tentang kategori ini..."><?= htmlspecialchars($_POST['category_description'] ?? '') ?></textarea>
                </div>
